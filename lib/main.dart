...
class MainApp extends StatelessWidget {
  ...
  Widget buttonRow(BluetoothDevice device){
    return Flex(
      ...
      children: [
        if(!device.isConnected) Expanded(child: ElevatedButton(onPressed: ()=>CanBluetooth.instance.connect(device), child: const Text("Connect"))),
        if(device.isConnected) Expanded(child:ElevatedButton(onPressed: ()=>CanBluetooth.instance.disconnect(device), child: const Text("Disconnect")))
      ]);
  }
  ...
  Widget bluetoothDeviceRow(String deviceKey){
    ...
    return Padding(
      ...
      child: Flex(direction: Axis.horizontal,children: [
        const Expanded(flex: 10, child: Icon(Icons.bluetooth)),
  ...
  Widget scanningButton(){
    if(FlutterBluePlus.isScanningNow){
      return ElevatedButton(onPressed: () => CanBluetooth.instance.stopScan(), child:const Text("Stop Scanning"));
    }
    return ElevatedButton(onPressed: () => CanBluetooth.instance.scan(), child:const Text("Scan For Bluetooth"));
  }
  ...
  Widget messages(){
    ...
    builder: (context,child){
      if(CanBluetooth.instance.deviceData.isEmpty) return const SizedBox();
      if(CanBluetooth.instance.connectedDevices.isEmpty) return const SizedBox();
      if(!CanBluetooth.instance.deviceData.containsKey(CanBluetooth.instance.connectedDevices.last)) return const SizedBox();
      debugPrint(CanBluetooth.instance.deviceData.toString());
      return ListView.separated(
      itemCount: CanBluetooth.instance.deviceData[CanBluetooth.instance.connectedDevices.last]!.length,
      separatorBuilder: (context, index) => const Divider(),
      itemBuilder: (context, index) => Text(CanBluetooth.instance.deviceData[CanBluetooth.instance.connectedDevices.last]!.values.elementAt(index).data.toString()),
      );
    });
  }
